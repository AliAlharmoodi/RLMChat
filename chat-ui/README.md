...
### Recursive Language Models (Optional)

This repo can run replies through the upstream RLM runtime when a local checkout of [`../rlm`](../rlm/README.md) is present. Instead of selecting separate `RLM · ...` models, you pick a normal base model and enable RLM mode in the chat UI. You can optionally choose a separate worker model for recursive sub-calls.
...
RLM_SUBCALL_MODEL=openai/gpt-4.1-mini
RLM_MAX_DEPTH=2
RLM_MAX_ITERATIONS=12
RLM_MAX_TIMEOUT_S=120
```

Notes:

- OpenRouter compatibility comes from the same `OPENAI_BASE_URL` / `OPENAI_API_KEY` settings used by normal models.
- The RLM bridge uses `uv run --project ../rlm`, so the local `rlm` directory must exist.
- The root model is the main chat model; `RLM_SUBCALL_MODEL` can point at a smaller worker model for recursion.
- RLM sessions are conversation-scoped and persist across turns, with durable restoration after server restarts.
- The current UI surfaces lightweight RLM progress and expandable execution traces in-chat.

<script lang="ts">
	...
	import InlineModelSelector from "./InlineModelSelector.svelte";
	...
	let streamingRlmStatus = $derived.by(() => {
		const updates = streamingAssistantMessage?.updates ?? [];
		for (let i = updates.length - 1; i >= 0; i -= 1) {
			const update = updates[i];
			if (isMessageRlmTraceStatusUpdate(update)) {
	...
	let candidateModels = $derived(models.filter((model) => !model.unlisted && !model.isRouter));
	let currentConversationId = $derived(page.params.id ?? "");
	let currentRlmMode = $state(false);

	$effect(() => {
		currentRlmMode = rlmMode;
	});

	let workerLabel = $derived.by(() => {
		if (!currentRlmMode) return "Direct";
		if (!$settings.rlmSubcallModel) return "Same as root";
		return (
			candidateModels.find((model) => model.id === $settings.rlmSubcallModel)?.displayName ??
			$settings.rlmSubcallModel
		);
	});

	async function patchConversationModelSettings(update: { model?: string; usesRlm?: boolean }) {
		if (!currentConversationId) return;
		await fetch(`${base}/conversation/${currentConversationId}`, {
			method: "PATCH",
	...
	async function handleRootModelChange(modelId: string) {
		await settings.instantSet({ activeModel: modelId });
		if (currentConversationId && !isReadOnly) {
			await patchConversationModelSettings({ model: modelId });
			currentModel = findCurrentModel(models, [], modelId);
		}
	}

	async function handleWorkerModeChange(value: string) {
		if (value === "__direct__") {
			currentRlmMode = false;
			await settings.instantSet({ rlmMode: false, rlmSubcallModel: "" });
			if (currentConversationId && !isReadOnly) {
				await patchConversationModelSettings({ usesRlm: false });
			}
			return;
		}
		...
		if (value === "__same__") {
			currentRlmMode = true;
			await settings.instantSet({ rlmMode: true, rlmSubcallModel: "" });
			if (currentConversationId && !isReadOnly) {
				await patchConversationModelSettings({ usesRlm: true });
			}
			return;
		}

		currentRlmMode = true;
		await settings.instantSet({ rlmMode: true, rlmSubcallModel: value });
		if (currentConversationId && !isReadOnly) {
			await patchConversationModelSettings({ usesRlm: true });
		}
	}
		...
		<div class="w-full">
							...
							>
								{#snippet controls()}
									<InlineModelSelector
										...
										label="Root"
										selectedLabel={currentModel.displayName}
										placeholder="Search root model"
										disabled={loading || isReadOnly}
										onchange={handleRootModelChange}
									/>
									<InlineModelSelector
										models={candidateModels}
										value={!currentRlmMode ? "__direct__" : ($settings.rlmSubcallModel || "__same__")}
										label="Worker"
										...
										extraOptions={[
											{
												value: "__direct__",
												label: "Direct",
												description: "Standard chat without the recursive RLM runtime.",
											},
											{
												value: "__same__",
												label: "Same as root",
												description: "Use the root model for recursive subcalls too.",
											},
										]}
										onchange={handleWorkerModeChange}
									/>
								{/snippet}
							</ChatInput>
						{/if}
			...
			>
				{#if models.find((m) => m.id === currentModel.id)}
					...
					{:else if loading && streamingRlmStatus}
						<span class="inline-flex items-center gap-1 whitespace-nowrap text-xs text-gray-400/85">
							<span class="font-medium">{streamingRlmStatus}</span>
						</span>

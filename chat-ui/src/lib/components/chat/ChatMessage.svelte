<script lang="ts">
	...
	import RlmTraceUpdate from "./RlmTraceUpdate.svelte";
	import {
		isMessageRlmTraceUpdate,
		isMessageToolUpdate,
	...
	type Block =
		...
		| { type: "rlmTrace"; updates: MessageRlmTraceUpdate[] };
	...
	let blocks = $derived.by(() => {
		...
		for (const update of updates) {
			...
			} else if (isMessageRlmTraceUpdate(update)) {
				const existingBlock = res.find((b): b is Extract<Block, { type: "rlmTrace" }> => b.type === "rlmTrace");
				if (existingBlock) {
					existingBlock.updates.push(update);
				} else {
					res.push({
						type: "rlmTrace" as const,
						updates: [update],
					});
				}
	...
	let hasRlmTrace = $derived(blocks.some((entry) => entry.type === "rlmTrace"));
...
{#if message.from === "assistant"}
	<div
		...
		class="group relative -mb-4 flex {hasRlmTrace ? 'w-full max-w-3xl' : 'w-fit max-w-full'} items-start justify-start gap-4 pb-4 leading-relaxed max-sm:mb-1 {message.routerMetadata &&
	...
	>
		<MessageAvatar
			classNames="{hasRlmTrace ? 'mt-6 size-3.5 shadow-none' : 'mt-5 size-3.5 shadow-lg'} flex-none select-none rounded-full max-sm:hidden"
			animating={isLast && loading}
		/>
		<div
			class="relative flex min-w-[60px] flex-col gap-2 break-words text-gray-600 prose-pre:my-2 dark:text-gray-300 {hasRlmTrace
				? 'w-full border-0 bg-transparent px-0 py-0'
				: 'rounded-2xl border border-gray-100 bg-gradient-to-br from-gray-50 px-5 py-3.5 dark:border-gray-800 dark:from-gray-800/80'}"
		>
		...
		{/if}
		{#each blocks as block, blockIndex (block.type === "tool" ? `${block.uuid}-${blockIndex}` : block.type === "rlmTrace" ? `rlm-trace-${blockIndex}` : `text-${blockIndex}`)}
			...
			{@const nextBlockHasThink =
				...
				{:else if block.type === "rlmTrace"}
					<div data-exclude-from-copy class="w-full">
						<RlmTraceUpdate
							trace={block.updates}
							{loading}
						/>
		...
		{#if !hasRlmTrace && (message.routerMetadata || (!loading && message.content))}
			<div

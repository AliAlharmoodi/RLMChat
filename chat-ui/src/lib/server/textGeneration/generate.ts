...
export async function* generate(
	...
	for await (const output of stream) {
		if ("rlmTrace" in output && output.rlmTrace) {
			if (output.rlmTrace.kind === "status") {
				yield {
			...
			yield {
				type: MessageUpdateType.RlmTrace,
				subtype: MessageRlmTraceUpdateType.Section,
				sectionId: output.rlmTrace.sectionId,
				title: output.rlmTrace.title,
				status: output.rlmTrace.status,
				summary: output.rlmTrace.summary,
				durationSeconds: output.rlmTrace.durationSeconds,
				details: output.rlmTrace.details,
			};
			continue;
		}

...
import { makeRlmEndpoint } from "$lib/server/rlm/endpoint";
...
const makeRlmWrappedModel = (baseModel: ProcessedModel): ProcessedModel => {
	const wrapped: ProcessedModel = {
		...baseModel,
		isRlm: true,
		rlmBaseModelId: baseModel.id,
		rlmBaseDisplayName: baseModel.displayName,
		getEndpoint: async (): Promise<Endpoint> => makeRlmEndpoint(wrapped),
	};

	return wrapped;
};

export const findBaseModelById = (modelId: string) =>
	models.find((model) => model.id === stripLegacyRlmPrefix(modelId));

export const resolveConversationModel = ({
	modelId,
	useRlm = false,
}: {
	modelId: string;
	...
	const baseModel = findBaseModelById(modelId);
	if (!baseModel) {
		return undefined;
	}

	if (rlmEnabled && (useRlm || isLegacyRlmModelId(modelId))) {
		return makeRlmWrappedModel(baseModel);
	}
...
const signatureForModel = (model: ProcessedModel) =>
	JSON.stringify({
		...
		isRlm: model.isRlm ?? false,
		rlmBaseModelId: model.rlmBaseModelId,

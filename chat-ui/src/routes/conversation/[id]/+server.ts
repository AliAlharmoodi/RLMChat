...
import { isLegacyRlmModelId } from "$lib/utils/models";
...
export async function POST({ request, locals, params, getClientAddress }) {
	...
	const model = resolveConversationModel({
		modelId: conv.model,
		useRlm:
			Boolean((conv as { usesRlm?: boolean }).usesRlm) || isLegacyRlmModelId(conv.model),
	});
	...
	const stream = new ReadableStream({
		async start(controller) {
			...
			try {
				...
				locals.userSettings = {
					rlmSubcallModel: userSettings?.rlmSubcallModel ?? "",
				};
...
export async function PATCH({ request, locals, params }) {
	...
	const updateValues = {
		...
		...(values.usesRlm !== undefined && { usesRlm: values.usesRlm }),
	};

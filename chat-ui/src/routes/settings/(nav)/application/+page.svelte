<script lang="ts">
	...
	let rlmCandidateModels = $derived(
		((page.data.models ?? []) as Model[]).filter((model) => !model.unlisted && !model.isRouter)
	);
			...
			<div class="divide-y divide-gray-200 dark:divide-gray-700">
				...
				<div class="flex items-start justify-between py-3">
					<div>
						...
						<p class="text-[12px] text-gray-500 dark:text-gray-400">
							Use the Recursive Language Model runtime for new conversations while keeping
							the same base model selection.
						</p>
					</div>
				...
				{#if $settings.rlmMode}
					<div class="flex items-start justify-between py-3">
						<div>
							...
							<p class="text-[12px] text-gray-500 dark:text-gray-400">
								Choose the smaller worker model for recursive subcalls. Leave it on
								“same as root model” if you want a single-model RLM setup.
							</p>
						</div>
						<div class="max-w-[20rem]">
							<ModelPicker
								...
								emptyDescription="Use the current root model for recursive subcalls too."

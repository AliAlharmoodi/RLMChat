...
const settingsSchema = z.object({
	...
	rlmMode: z.boolean().default(DEFAULT_SETTINGS.rlmMode),
	rlmSubcallModel: z.string().default(DEFAULT_SETTINGS.rlmSubcallModel),
...
export const GET: RequestHandler = async ({ locals }) => {
	...
	if (settings?.activeModel && isLegacyRlmModelId(settings.activeModel)) {
		settings.activeModel = stripLegacyRlmPrefix(settings.activeModel);
		settings.rlmMode = true;
		await collections.settings.updateOne(authCondition(locals), {
			$set: { activeModel: settings.activeModel, rlmMode: true },
		});
	}
	...
	if (
		...
		settings.rlmSubcallModel !== DEFAULT_SETTINGS.rlmSubcallModel &&
		!validateModel(models).safeParse(settings.rlmSubcallModel).success
	) {
		settings.rlmSubcallModel = DEFAULT_SETTINGS.rlmSubcallModel;
		await collections.settings.updateOne(authCondition(locals), {
			$set: { rlmSubcallModel: DEFAULT_SETTINGS.rlmSubcallModel },
		});
	}
	...
	return superjsonResponse({
		...
		rlmMode: settings?.rlmMode ?? DEFAULT_SETTINGS.rlmMode,
		rlmSubcallModel: settings?.rlmSubcallModel ?? DEFAULT_SETTINGS.rlmSubcallModel,
		streamingMode,
